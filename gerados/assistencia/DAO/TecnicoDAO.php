...
class TecnicoDAO{
	...
	public function inserir($tecnico){
		include("conexao.php");
		$sql = 'INSERT INTO tecnico (id, nome, telefone, sexo, senha, email, data_nasc, categoria, preco, data_entrega) VALUES (:id, :nome, :telefone, :sexo, :senha, :email, :data_nasc, :categoria, :preco, :data_entrega)';
		$consulta = $conexao->prepare($sql);
...
		$consulta->bindValue(':preco',$tecnico->getPreco()); 
		$consulta->bindValue(':data_entrega',$tecnico->getData_entrega()); 
		if($consulta->execute())
			return true;
		else
			return false;
	...
	public function atualizar($tecnico){
		include("conexao.php");
		$sql = 'UPDATE tecnico SET id = :id, nome = :nome, telefone = :telefone, sexo = :sexo, senha = :senha, email = :email, data_nasc = :data_nasc, categoria = :categoria, preco = :preco, data_entrega = :data_entrega WHERE id = :id';
		$consulta = $conexao->prepare($sql);
		$consulta->bindValue(':id',$tecnico->getId()); 
